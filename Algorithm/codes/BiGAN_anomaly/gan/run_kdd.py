...
def train_and_test(nb_epochs, weight, method, degree, random_seed):
    ...
    input_pl = tf.placeholder(
        tf.float32, shape=data.get_shape_input(), name="input")
    ...
    display_parameters(batch_size, starting_lr,
                       ema_decay, weight, method, degree)

    gen = network.generator
    dis = network.discriminator

    # Sample noise from random normal distribution
    random_z = tf.random_normal(
        [batch_size, latent_dim], mean=0.0, stddev=1.0, name='random_z')
    ...
    fake_d, inter_layer_fake = dis(
        generator, is_training=is_training_pl, reuse=True)
    ...
    with tf.name_scope('loss_functions'):
        ...
        real_discriminator_loss = tf.losses.sigmoid_cross_entropy(
            tf.constant(1, shape=[batch_size]), real_d, scope='real_discriminator_loss')
        fake_discriminator_loss = tf.losses.sigmoid_cross_entropy(
            tf.constant(0, shape=[batch_size]), fake_d, scope='fake_discriminator_loss')
    ...
    with tf.name_scope('optimizers'):
        ...
        dvars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope='discriminator')
        gvars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator')

        update_ops_gen = tf.get_collection(
            tf.GraphKeys.UPDATE_OPS, scope='generator')
        update_ops_dis = tf.get_collection(
            tf.GraphKeys.UPDATE_OPS, scope='discriminator')

        optimizer_dis = tf.train.AdamOptimizer(
            learning_rate=learning_rate, beta1=0.5, name='dis_optimizer')
        optimizer_gen = tf.train.AdamOptimizer(
            learning_rate=learning_rate, beta1=0.5, name='gen_optimizer')

        # attached op for moving average batch norm
    ...
    with tf.variable_scope("latent_variable"):
        z_optim = tf.get_variable(name='z_optim', shape=[
                                  batch_size, latent_dim], initializer=tf.truncated_normal_initializer())
        reinit_z = z_optim.initializer
    # EMA
    generator_ema = gen(z_optim, is_training=is_training_pl,
                        getter=get_getter(gen_ema), reuse=True)
    # Pass real and fake images into discriminator separately
    real_d_ema, inter_layer_real_ema = dis(
        input_pl, is_training=is_training_pl, getter=get_getter(gen_ema), reuse=True)
    fake_d_ema, inter_layer_fake_ema = dis(
        generator_ema, is_training=is_training_pl, getter=get_getter(gen_ema), reuse=True)
    ...
    with sv.managed_session() as sess:
        ...
        while not sv.should_stop() and epoch < nb_epochs:
            ...
            for t in range(nr_batches_train):
                ...
                feed_dict = {input_pl: trainx[ran_from:ran_to],
                             ...
                             learning_rate: lr}
                _, ld, sm = sess.run(
                    [train_dis_op, discriminator_loss, sum_op_dis], feed_dict=feed_dict)
                train_loss_dis += ld
                ...
                feed_dict = {input_pl: trainx_copy[ran_from:ran_to],
                             ...
                             learning_rate: lr}
                _, lg, sm = sess.run(
                    [train_gen_op, generator_loss, sum_op_gen], feed_dict=feed_dict)
                train_loss_gen += lg
                writer.add_summary(sm, train_batch)
        ...
        for t in range(nr_batches_test):
            ...
            print("\rTesting batch %d/%d, inference time %.4f" %
                  (t, nr_batches_test, time.time()-begin_val_batch), end=".")
